...
async function ensureSchema() {
  if (!schemaPromise) schemaPromise = database().query(`
    ...
    CREATE TABLE IF NOT EXISTS players (id uuid PRIMARY KEY, user_id uuid UNIQUE REFERENCES users(id) ON DELETE CASCADE, player_id text UNIQUE NOT NULL, passcode text UNIQUE NOT NULL, name text NOT NULL, position text, initials text, avatar text, created_at timestamptz NOT NULL DEFAULT now());
  ...
  `).then(() => database().query("ALTER TABLE players ADD COLUMN IF NOT EXISTS avatar text")).catch((error) => { schemaPromise = null; throw error; });
...
app.get("/api/players", auth, route(async (_req, res) => { await ensureSchema(); res.json((await database().query('SELECT id, player_id AS "playerId", passcode, name, position, initials, avatar FROM players ORDER BY name')).rows); }));
...
app.post("/api/payment-requests", auth, route(async (req, res) => { await ensureSchema(); const { player, playerId, amount, date, reference = "", evidence } = req.body; if (req.user.role !== "admin" && !playerId) return res.status(400).json({ error: "Player is required" }); if (!evidence || !String(evidence).startsWith("data:image/")) return res.status(400).json({ error: "Screenshot evidence is required" }); const result = await database().query('INSERT INTO payment_requests (id,player,player_id,amount,date,reference,evidence) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING id,player,player_id AS "playerId",amount,date,reference,evidence', [randomUUID(), player, playerId || null, amount, date, reference, evidence]); res.status(201).json(result.rows[0]); }));
...
app.get("/api/settings", auth, route(async (_req, res) => { await ensureSchema(); const result = await database().query("SELECT key,value FROM settings WHERE key IN ('fundRequirement','easyPaisaNumber','clubLogo','wallpaper','adminProfileImage')"); res.json(Object.fromEntries(result.rows.map((r) => [r.key, r.value]))); }));
...
app.put("/api/profile", auth, route(async (req, res) => { await ensureSchema(); const avatar = req.body?.avatar ? String(req.body.avatar) : ""; if (avatar && !avatar.startsWith("data:image/")) return res.status(400).json({ error: "Profile image must be an image" }); if (req.user.role === "admin") { await database().query("INSERT INTO settings (key,value) VALUES ('adminProfileImage',$1) ON CONFLICT (key) DO UPDATE SET value=EXCLUDED.value", [avatar]); } else { await database().query("UPDATE players SET avatar=$1 WHERE player_id=$2", [avatar, req.user.playerId]); } res.json({ avatar }); }));
